...
class SatiEncoder(nn.Module):
    def __init__(
        ...
        elif obs_encoder("-")[0] == "dinov2":
            self.obs_encoder = DiNOV2(
                backbone_size=obs_encoder.split("-")[1]
            
            )
            self.num_obs_features = self.obs_encoder._fc.in_features
            self.obs_encoder_type = "dinov2"
        else:
